...
export default function OnePost(props) {
    ...
    const handleLikeToggle = async () => {
        ...
        let response = await fetch('/like-post', {method: 'POST', body: data})
    ...
    const handleComment = async event => {
        ...
        data.append("post", props.item._id) 
        let response = await fetch('/new-comment', {method: 'POST', body: data})
        let body = await response.text()
        let parsed = JSON.parse(body)
        if (parsed.success) {
            dispatch({type: "SET-FEED", content: parsed.feed})
    ...
    const handleDeleteComment = async comment => {
        ...
        data.append("post", props.item._id) 
        let response = await fetch('/delete-comment', {method: 'POST', body: data})
        let body = await response.text()
        let parsed = JSON.parse(body)
        if (parsed.success) {
            dispatch({type: "SET-FEED", content: parsed.feed})
